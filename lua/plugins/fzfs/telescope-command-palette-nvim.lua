...
M.config = function()
	...
	CpMenu = {
		{
			"File",
			{ "entire selection (C-a)",  '<cmd>call feedkeys("GVgg")' },
	...
	table.insert(CpMenu, {
		...
		{ "current scopes",                 '<cmd>lua ViewCurrentScopes(); vim.cmd("wincmd w|vertical resize 40")' },
		...
		{ "repl",                           "<cmd>lua require'dap'.repl.open(); vim.cmd(\"wincmd w|resize 12\")" },
