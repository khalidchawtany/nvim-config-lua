...
_G.get_statuscol_gitsign = function(bufnr, lnum)
    ...
    -- local ft = vim.api.nvim_get_option_info2("filetype", { buf = bufnr }).value
    if vim.startswith(bufname, "fugitive") then
        return " "
    end
    -- if vim.fn.getbufinfo(bufnr) == 0 then
    --     return ""
    -- end
    ...
    for _, mark in ipairs(vim.api.nvim_buf_get_extmarks(bufnr, _G.git_signs_name_space, 0, -1, { details = true })) do
        if mark[1] == lnum then
